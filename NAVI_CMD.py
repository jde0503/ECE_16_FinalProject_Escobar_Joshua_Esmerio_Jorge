...
arduinoSerial = serial.Serial(arduinoPorts[0], baudrate=115200, timeout=0.02)
...
while (True):
    if (arduinoSerial.in_waiting):
        ...
        if ((currentOrientation is not None) and
        ...
        leftCalfFiltered = sig.lfilter(b_low, a_low, leftCalfRaw)
        leftCalfFiltered = sig.lfilter(b_high, a_high, leftCalfFiltered)
        leftCalfFiltered = np.absolute(leftCalfFiltered)
        leftCalfFiltered = sig.lfilter(box, 1, leftCalfFiltered)

        rightCalfFiltered = sig.lfilter(b_low, a_low, rightCalfRaw)
        rightCalfFiltered = sig.lfilter(b_high, a_high, rightCalfFiltered)
        rightCalfFiltered = np.absolute(rightCalfFiltered)
        rightCalfFiltered = sig.lfilter(box, 1, rightCalfFiltered)

        # If timing is right, check for flexes.
        if ((time.perf_counter() - timeOfLastFlex) > TIME_BETWEEN_FLEXES):
            leftFlexDetected = isFlexPresent(leftCalfFiltered[-SAMPLE_RATE:])
            rightFlexDetected = isFlexPresent(rightCalfFiltered[-SAMPLE_RATE:])
